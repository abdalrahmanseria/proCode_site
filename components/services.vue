<template>
    ...
    <!-- Service Dialog -->
    <div :class="isDialogOpen ? 'fixed' : 'hidden'" class="bg-black/[0.9] top-0 left-0 bottom-0 w-[100%] h-[100%] z-[9999] overflow-y-auto">
        <div class="min-h-[100%] flex items-center justify-center p-4">
            <div class="relative bg-white dark:bg-slate-900 rounded-lg shadow-2xl max-w-6xl w-full my-8">
                <!-- Close Button -->
                <button @click="closeServiceDialog" class="absolute top-4 end-4 text-slate-400 hover:text-slate-900 dark:hover:text-white text-2xl z-10">
                ...
                <div class="p-6 md:p-8">
                    ...
                    <div class="prose dark:prose-invert max-w-none mb-6">
                        <p class="text-slate-600 dark:text-slate-300 text-lg leading-relaxed">{{ selectedService?.fullDescription }}</p>
                    </div>
                    
                    <!-- Images Gallery -->
                    <div v-if="selectedService?.images && selectedService.images.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8">
                        <div v-for="(img, index) in selectedService.images" :key="index" class="relative group cursor-pointer overflow-hidden rounded-lg">
